 ...
 * server pages (homepage) and the client-rendered catalogue grid. The heart sits
 * beside the link (not inside it) and floats over the photo's top-right corner.
 */
export function JerseyCard({ product, priority = false }: { product: CatalogueProduct; priority?: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      <FavouriteButton productId={product.id} productName={product.name} className="absolute right-2 top-2" />
    </div>
  );
}
